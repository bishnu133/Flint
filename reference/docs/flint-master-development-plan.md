# Flint — Master Development Plan v1.0
...
Flint generates production-quality Playwright TypeScript automation suites for any web application, from three inputs:
...
The generated suite is plain Playwright TS (Page Object Model) that runs with `npx playwright test` and has **zero runtime dependency on Flint**.
...
- Storage: plain JSON files under `.flint/` (no DB in V1); everything versioned & diffable in git
...
│   ├── config/                 # config load + zod schema (flint.config.ts in target project)
...
└── tests/                      # vitest unit + integration tests for Flint itself
```

Target project layout (created by `flint init`):
...
Each stage: pure module, typed input/output, callable via CLI independently, and re-runnable (idempotent). `flint ci` chains all five.
...
**Build:** repo scaffold, strict TS config, CLI skeleton with all command stubs, config schema + loader, ALL zod schemas from B4, LLMProvider interface + Anthropic implementation + call logger, prompt template loader (md files with `{{placeholders}}`), `flint init` (scaffolds kb/, config, e2e skeleton from templates).
**Expect at exit:** `flint init` produces a valid project; `flint --help` lists all commands; schemas have unit tests incl. rejection cases; a `hello-llm` smoke command proves provider wiring.
...
**Build:** ts-morph static scan of `e2e/`: page-object classes/methods, spec files/test titles/tags, fixtures, data factories; coverage map from `@feature:<id>` tags + plan history; managed-file detection via `/* @flint:managed <hash> */` header (hash of generated content → hand-edit detection by hash mismatch); `flint index` command + JSON output.
...
**Build:** dialect interface + `playwright-pom` dialect; POM emitter (one class per Screen Model page used, selectors from top-ranked candidates only, methods derived from plan steps); fixture emitter (auth via storageState fixture; data factory stubs from `dataNeeds`); spec emitter (describe per feature, test per plan case, tags incl. `@flint @feature:<id>`); reuse enforcement (if Suite Index has `LoginPage`, extend it via new methods in a partial-class-safe way: emit additions with managed markers into `login.page.flint.ts` companion or in-place if the file is fully managed); `tsc --noEmit` + eslint gate before writing to suite; `flint generate <feature> [--dry-run]` with diff preview.
...
**Build:** runner (spawns `npx playwright test --reporter=json` scoped to generated tags/files, env from config); failure classifier (deterministic: match Playwright error shapes → failureClass; attach trace/screenshot paths); repair loop (per failing test: structured failure context — failureClass, error excerpt, plan step, current code, relevant Screen Model elements — → repair prompt → patched code → re-run just that test; max 2 iterations LOCKED; selector-not-found failures first retry deterministically with next selector candidate BEFORE any LLM call); fixme fallback with comment block (failureClass + last error + repair history); RunReport writer; `flint verify [--repair]`.
...
**Scenarios:** app genuinely broken (real bug!) — assertion-mismatch failures that survive repair are surfaced as "possible application defect" in the report, distinct from test defects (this is a feature: Flint finding real bugs); test data collisions between parallel tests (report recommends serial mode or data factory uniqueness — factories emit unique suffixes by default); auth expiry mid-run.

### Phase 6 — Integration, CI & Polish (1.5 wks)

**Build:** `flint ci` (chained pipeline, headless, exits non-zero on failures, machine-readable summary); drift mode (`flint explore --diff` → map changed elements to affected tests via Suite Index selector usage → report "these 7 tests likely break" → optional targeted regeneration of page objects only); GitHub PR mode (branch, commit, PR body = rendered plan + run report; `gh` CLI or octokit); docs (README, KB authoring guide, config reference); benchmark runner (metrics: compile rate, first-run pass, post-repair pass, selector re-resolve rate, tokens + est. cost per feature, wall time per stage) with baseline recorded; **stretch:** `bubblegum` dialect.
**Expect at exit:** end-to-end: drop new feature spec → `flint ci` → PR with passing tests, no human touch in between; benchmark baseline documented; a stranger can onboard from README alone.
...
- **Memory layer:** persistent project memory (JSON/SQLite under `.flint/memory/`): which selector strategies survive longest in THIS app, which flows are flaky, past repair diagnoses, per-page quirks ("date picker needs keyboard input"). Injected into relevant prompts. This is how the system improves with time.

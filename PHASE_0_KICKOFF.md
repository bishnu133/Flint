# Phase 0 Kickoff — TestGen Foundation

Paste this as your first message to Claude Code (or run it as a command) after the repo is set up with CLAUDE.md and docs/testgen-master-development-plan.md in place.

---

Read `CLAUDE.md` and `docs/testgen-master-development-plan.md` fully before writing any code.

We are starting **Phase 0 — Foundation** of TestGen. Scope is exactly Part C / Phase 0 of the master plan. Do not build anything from later phases.
...
2. **All shared zod schemas** in `src/schemas/` per master plan section B4: `screen-model.ts`, `test-plan.ts`, `suite-index.ts`, `run-report.ts`, `kb.ts` (feature-spec frontmatter), plus `config.ts` (testgen.config schema: baseUrl, envClass, auth modes, explorer options incl. urlPattern rules / maxPages / maxDepth / dangerousActionPatterns / i18n flag / roles, models per role, dialect, token budgets, debug flags). Every schema gets unit tests including at least 2 rejection cases with assertion on the error message quality.
...
6. **`testgen init`**: scaffolds target-project layout per B2 (testgen.config.ts, kb/ starter files from templates/, e2e/ skeleton with playwright.config.ts + auth fixture stub + tsconfig). Re-running on an existing project must prompt and never clobber.
7. **`testgen hello-llm`**: smoke command proving provider wiring (one tiny structured call, prints model + token counts).

## Exit criteria (verify and report each with evidence)

- `pnpm build`, `pnpm lint`, `pnpm test` all clean — state final test count
- `testgen --help` lists all 8 commands
- `testgen init` run in a temp dir produces a valid project; re-run prompts instead of overwriting
- `testgen hello-llm` works with ANTHROPIC_API_KEY set, and fails with an actionable error naming the env var when unset
- Invalid testgen.config.ts produces a friendly zod error naming the bad key, not a stack trace
